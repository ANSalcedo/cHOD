...
hostDMH * find_galaxy_hosts(struct halo halos[], halo_metadata * env, double siglogM, double logMmin, double q_cen, double alpha_cen, double redshift, unsigned long int N_h, unsigned long int *Ncen, gsl_rng *r)
{
  ...
  for(i = 0; i < N_h; i++)
      ...
      if(prob > gsl_rng_uniform(r))
	{
	  ...
	  hosts[INDEX10(j,3)] = halos[i].vx + gsl_ran_laplace(r, alpha_cen*vrms*pow(2, -1.0/6.0)); /* extra power of root 3 comes from conversion of 3D dispersion to 1D */ 
	  hosts[INDEX10(j,4)] = halos[i].vy + gsl_ran_laplace(r, alpha_cen*vrms*pow(2, -1.0/6.0));
	  hosts[INDEX10(j,5)] = halos[i].vz + gsl_ran_laplace(r, alpha_cen*vrms*pow(2, -1.0/6.0));
